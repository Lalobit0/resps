...
export function gruposDuplicados(equipos) {
  ...
  for (const [clave, lista] of porClave) {
    ...
    if (clave.startsWith("S")) {
      // Una "serie" que traen tres o más equipos, o que sigue el patrón de los
      // códigos de modelo (PBBJ0029MX, PAWR0044MX), no identifica al aparato.
      // Tampoco se unen dos registros con IMEI distintos. Y si uno trae IMEI y
      // el otro no, no hay forma de probar que sean el mismo: se dejan aparte,
      // porque unirlos borraría un teléfono real del inventario.
      const conImei = lista.map((e) => dig(e.det.imei)).filter((x) => x.length >= 10);
      const imeis = new Set(conImei);
      const esCodigoModelo = lista.length > 2 || /^S(PBBJ|PAWR)/.test(clave);
      const mezclaSinImei = conImei.length > 0 && conImei.length < lista.length;
      if (esCodigoModelo || imeis.size > 1 || mezclaSinImei) {
        avisos.push(
          `${clave.slice(1)} la comparten ${lista.length} equipos (${lista.map((e) => e.codigo).join(", ")}): ` +
